...
Now that **_your_** node has a copy of this file, we can view it via our **local IPFS gateway** - this is a **HTTP server for IPFS**. So we can use this _webpage_ to interact with IPFS!
...
So by following along here - you just advertized your node (by your _peer ID_) to other nodes you are connected to that you have and will share this data ... if they ask nicely ;)
...
[^1]: By default, yes. Only if you [**pin**](https://docs.ipfs.io/how-to/pin-files/) this data (file upload does this automatically). But this data will only be shared if other peers know the CIDs for content you hold and request it. And only if your machine is on a network that can connect _out to other peers_ - and many firewalls and NATs don't always play nice with this.
...
Static sites are just a set of HTML, CSS, and JS files... **so once created, we can _host_ static sites on IPFS!!**
